...
for vidname in vidlist:
    ...
    stillgrowing = False
    while(threshold <= thresholdlimit or stillgrowing):
        print("----"+str(round(threshold/thresholdlimit*100)) + " %")
        start = 0
        end = 0
        cutlist = []
        
        while start < len(data) - bufferduration - 5:
            if np.mean(data[start:start+5]) < limit + offset:
                start = start + 1
                end = start
            else:
        ...
        if a > bestP:
            ...
            bestCutlist = cutlist
            stillgrowing = True
            if threshold >= thresholdlimit-30:
                thresholdlimit += 30
        else:
            stillgrowing = False
